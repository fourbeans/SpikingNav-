#!/usr/bin/env python3
"""
使用 BSN 的模拟流水线（不依赖 AllenAct），BSN 输出二值化版

该脚本基于 simulate_goal_visual_encoder_bsn.py，区别是所有 BSN 层
支持 `binary_out` 参数，默认在命令行中启用以产生 0/1 输出并打印认证信息。
"""
import argparse
import os
import numpy as np
...
# try to import SpikeCountBinaryLIFNode from local neuron.py; fallback to None
try:
    from neuron import SpikeCountBinaryLIFNode
except Exception:
    SpikeCountBinaryLIFNode = None


class BSN(nn.Module):
    """Basic Spiking Neuron (BSN) with optional binary output.

    Args:
        T: int - time steps
        neuron_size: int - output channels
        m_weight: float - memory weight
        binary_out: bool - whether to collapse to binary 0/1 per element
        collapse_mode: str - 'any'|'majority'|'threshold_k'
        threshold_k: int - used when collapse_mode == 'threshold_k'
        verbose: bool - print diagnostics when True
    """

    def __init__(self, T: int, neuron_size=1, m_weight=0.1, binary_out: bool = False,
                 collapse_mode: str = 'any', threshold_k: int = 1, target_density: float = 0.3, verbose: bool = False):
        super().__init__()
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        # increase time dim and repeat to T, compute multispike, then return mean over time
        ...
        output_seq = spike.view(x_seq.shape)  # [T, B, C, ...]

        if not self.binary_out:
            return output_seq.mean(0)

        # binary_out: compute statistics before conversion
        # pre_nonzero_ratio: 比例 = 在 T 个时间步中非0（有发放）步数占比
        pre_nonzero = (output_seq != 0).to(torch.float32).mean(dim=0)  # [B, C, ...], in [0,1]
        pre_one_ratio = float(pre_nonzero.mean().detach().cpu().item())

        # If collapse_mode is 'percentile', compute binary by selecting top elements
        if self.collapse_mode == 'percentile':
            # ensure density in [0,1]
            target = min(max(self.target_density, 0.0), 1.0)
            flat = pre_nonzero.flatten()
            if flat.numel() == 0:
                thr = 1.0
            else:
                try:
                    thr = torch.quantile(flat, 1.0 - target)
                except Exception:
                    # fallback: use sort kth
                    k = max(1, int(math.ceil((1.0 - target) * flat.numel())))
                    thr = torch.sort(flat)[0][k - 1]

            binary = (pre_nonzero >= thr).to(torch.float32)
            min_thr = 0.25
            binary = torch.logical_and(pre_nonzero >= thr, pre_nonzero >= min_thr).float()

            if self.verbose:
                try:
                    p10 = float(torch.quantile(flat, 0.10).item())
                    p25 = float(torch.quantile(flat, 0.25).item())
                    ...
                    p10 = float(sorted_flat[int(0.10 * (sorted_flat.numel() - 1))].item()) if sorted_flat.numel() > 0 else 0.0
                    p25 = float(sorted_flat[int(0.25 * (sorted_flat.numel() - 1))].item()) if sorted_flat.numel() > 0 else 0.0
                    p50 = float(sorted_flat[int(0.50 * (sorted_flat.numel() - 1))].item()) if sorted_flat.numel() > 0 else 0.0
                    p75 = float(sorted_flat[int(0.75 * (sorted_flat.numel() - 1))].item()) if sorted_flat.numel() > 0 else 0.0
                    p90 = float(sorted_flat[int(0.90 * (sorted_flat.numel() - 1))].item()) if sorted_flat.numel() > 0 else 0.0

                print(f"BSN[id={hex(id(self))}]: pre_nonzero_mean={pre_one_ratio:.4f}, p10={p10:.3f}, p25={p25:.3f}, p50={p50:.3f}, p75={p75:.3f}, p90={p90:.3f}, thr={thr:.3f}, target_density={target:.3f}")
                print(f"BSN[id={hex(id(self))}]: out_unique={torch.unique(binary).cpu().numpy().tolist()}, one_ratio={(binary==1).float().mean().item():.4f}")

            return binary

        # default counts computation: two options available
        # - method 'rate' (legacy): use averaged amplitude -> round(rate*T)
        # - method 'spike_count': count actual non-zero steps per element
        if self.collapse_mode == 'rate':
            rate = output_seq.mean(dim=0)
            rate = torch.clamp(rate, 0.0, 1.0)
            counts = torch.round(rate * float(self.T)).clamp(0, self.T).long()
        else:
            # default/spike_count: count actual number of non-zero time steps
            counts = output_seq.gt(0).to(torch.long).sum(dim=0).clamp(0, self.T)

        # optional debug: expand counts back to spike seq using SpikeCountBinaryLIFNode
        if self.verbose and SpikeCountBinaryLIFNode is not None:
            try:
                lif = SpikeCountBinaryLIFNode()
                debug_seq = lif.forward(counts, T=self.T)
                unique_debug = torch.unique(debug_seq)
                debug_one_ratio = (debug_seq == 1).float().mean().item()
            except Exception:
                unique_debug = None
                debug_one_ratio = None
        else:
            unique_debug = None
            debug_one_ratio = None

        # collapse counts -> binary according to mode
        if self.collapse_mode == 'any':
            binary = (counts > 0).to(torch.float32)
        elif self.collapse_mode == 'majority':
            binary = (counts >= math.ceil(self.T / 2)).to(torch.float32)
        elif self.collapse_mode == 'threshold_k':
            k = max(1, int(self.threshold_k))
            binary = (counts >= k).to(torch.float32)
        elif self.collapse_mode == 'spike_count':
            # already the spike-count mapping -> treat any non-zero as 1
            binary = (counts > 0).to(torch.float32)
        else:
            # fallback to any
            binary = (counts > 0).to(torch.float32)
        ...
        if self.verbose:
            try:
                # print pre-conversion nonzero ratio and post-conversion one ratio
                print(f"BSN[id={hex(id(self))}]: pre_nonzero_one_ratio={pre_one_ratio:.4f}")
                if unique_debug is not None:
                    print(f"BSN[id={hex(id(self))}]: debug_unique={unique_debug.cpu().numpy().tolist()}, debug_one_ratio={debug_one_ratio:.4f}")
                # print counts distribution
                try:
                    flat_counts = counts.flatten()
                    hist = torch.bincount(flat_counts) if flat_counts.numel() > 0 else torch.tensor([])
                    hist_list = hist.cpu().numpy().tolist()
                    print(f"BSN[id={hex(id(self))}]: counts_hist={hist_list}")
                except Exception:
                    pass
                print(f"BSN[id={hex(id(self))}]: out_unique={torch.unique(binary).cpu().numpy().tolist()}, one_ratio={(binary==1).float().mean().item():.4f}")
            except Exception:
                print(f"BSN[id={hex(id(self))}]: one_ratio={(binary==1).float().mean().item():.4f}")

        return binary


# -------------------- ResNet Preprocessor / Encoder 等（复用原脚本实现） --------------------
...
def main():
    ...
    parser.add_argument("--collapse_mode", choices=['any','majority','threshold_k','rate','percentile','spike_count'], default='majority')
    parser.add_argument("--threshold_k", type=int, default=1)
    parser.add_argument("--target_density", type=float, default=0.4, help='Target density for percentile mode, in (0,1)')
    ...
    # enable binary output in the spiking neuron kwargs used to construct the ResNet
    pre.spiking_neuron = BSN
    pre.spiking_neuron_kwargs = {
        "T": args.T,
        "binary_out": True,
    ...
    # replace BSN instances inside encoder (best-effort)
    def replace_bsn(module):
        for name, child in module.named_children():
            if isinstance(child, BSN):
                # already our BSN; ensure binary_out set
                setattr(module, name, BSN(
                    T=args.T,
                    neuron_size=getattr(child, 'neuron_size', 1),
                    binary_out=True,
                    collapse_mode=args.collapse_mode,
                    threshold_k=args.threshold_k,
                    target_density=args.target_density,
