...
- 该脚本从已采集的 `collected.npz` 读取 RGB 帧（N,H,W,3 uint8），并模拟从
  `ResNetPreprocessor` 到 `ResnetTensorGoalEncoder` 的整个处理流程，使用仓库中风格的
  `BSN` 实现（复制自 `neurons_enco.py`），保证 `x = self.goal_visual_encoder(observations)`
  的输出在形状上与原实现一致（单帧情况下为 `(1,1,D)`，批量时为 `(1,B,D)`）。

用法：
python simulate_goal_visual_encoder_bsn.py --input projects/objectnav_baselines/data_collect/collected.npz \
  --out projects/objectnav_baselines/data_collect/encoded_x_bsn.pt --device cpu
...
# -------------------- BSN 及辅助实现（与仓库中 neurons_enco.py 等价） --------------------
...
# -------------------- ResNet Preprocessor（与 allenact.embodiedai.preprocessors.resnet_snn.ResNetPreprocessor 等价，但不继承 AllenAct） --------------------
...
class ResNetPreprocessorBSN:
    """行为上等价于 allenact 中的 ResNetPreprocessor，但不继承 Preprocessor。

    支持相同的初始化参数（包含 SEW 参数），并提供 `to(device)` 与 `process(obs)` 方法。
    """
...
# -------------------- ResnetTensorGoalEncoder（BSN 版，尽量与仓库一致） --------------------
